...
for f = 1 : MAX_FREQ_INDEX
    ...
    for i = 2 : MAX_CALIB_SAMPLES
        if abs_sum(i) > (20 * 0.2 + abs_sum(i - 1))
            % we found that jump
            % check next adjacent sample as well for comparison
            if abs_sum(i) > abs_sum(i+1)
                calibrated_samples_delay(f) = i;
            else
                calibrated_samples_delay(f) = i+1;
            end
            break;
        end
...
fprintf(fileID, '#ifndef FEEDBACK_CALIBRATION_DATA_H\n');
fprintf(fileID, '#define FEEDBACK_CALIBRATION_DATA_H\n');
...
fprintf(fileID, 'float feedback_mag_pct[%d][%d] = {\n', MAX_FREQ_INDEX, MAX_AMP_INDEX);
...
%write samples delay
fprintf(fileID, 'float feedback_samples_delay[%d] = {\n', MAX_FREQ_INDEX);

for f = 1 : MAX_FREQ_INDEX  
    if f < MAX_FREQ_INDEX
        fprintf(fileID, '%f,\n', calibrated_samples_delay(f));
    else
        fprintf(fileID, '%f\n', calibrated_samples_delay(f));
    end
end
...
fprintf(fileID, '#endif /* FEEDBACK_CALIBRATION_DATA_H */\n');
